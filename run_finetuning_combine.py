...
# importlib.reload(timm.loss.LabelSmoothingCrossEntropy)
...
from engines.engine_for_finetuning_combine import train_one_epoch, validation_one_epoch, final_test, merge, merge2
from utils import NativeScalerWithGradNormCount as NativeScaler
from utils import multiple_samples_collate
import utils
from models import *
from models.internvl_clip_vision import inflate_weight
from models import humansam
from models.depth_pro.depth_pro_encoder import create_depth_pro_encoder

# from visualizer import get_local
# get_local.activate()
# get_local.clear()

def load_pretrained_checkpoint(model, args):
    ...
    # Handle head mismatch for custom classes (e.g. 2 or 4)
    if 'head.weight' in checkpoint_model:
        print(f"Dropping head from checkpoint (classes: {checkpoint_model['head.weight'].shape[0]}) as requested.")
    ...
    if 'pos_embed' in checkpoint_model:
        ...
        # Default to 8 frames for pretraining if not specified
        orig_t_size = 8
...
def get_args():
    ...
    parser.add_argument('--filename_tmpl', default='frame_{:05}.jpg', type=str, help='file template')
    ...
    parser.add_argument('--no_use_decord', action='store_true', dest='use_decord')
    ...
    parser.add_argument('--return_depth', action='store_true')
    parser.add_argument('--no_return_depth', action='store_false', dest='return_depth')
    parser.set_defaults(return_depth=True)

    parser.add_argument('--depth_checkpoint_path', default=None, type=str,
                        help='path to depth pro checkpoint')

    parser.add_argument('--use_conf', action='store_true', help='Use confidence weighting for loss')
    parser.set_defaults(use_conf=False)
...
def main(args):
    ...
    print(f"DEBUG: args.nb_classes before create_model: {args.nb_classes}")
    ...
    if args.finetune:
        load_pretrained_checkpoint(model, args)
    ...
    if args.return_depth:
        ...
        #     model.load_state_dict(combined_checkpoint['model'])
    model.to(device)

    print(f"DEBUG: Model head: {model.head}")
    if hasattr(model.head, 'weight'):
        print(f"DEBUG: Model head weight shape: {model.head.weight.shape}")
    ...
    total_batch_size = args.batch_size * args.update_freq * accelerator.num_processes
    ...
    #     test_stats = final_test(data_loader_test, model, device, preds_file, bf16=args.bf16)
    #     # torch.distributed.barrier()
        
    #     if global_rank == 0:
    #         print("Start merging results...")
    #         final_top1, final_top5, final_precision, final_recall, final_auc, double_acc, double_auc = merge(
    #             args.output_dir, num_tasks)
    #         print(
    #             f"Accuracy of the network on the {len(dataset_test)} test videos: Top-1: {final_top1:.2f}%, Top-5: {final_top5:.2f}%, Precision: {final_precision:.2f}%, Recall: {final_recall:.2f}%, AUC: {final_auc:.2f}%")
    #         log_stats = {'Final top-1': final_top1,
    #                      'Final AUC': final_auc,
    #                      'Double ACC': double_acc,
    #                      'Double AUC': double_auc,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            bf16=args.bf16, accelerator=accelerator, use_conf=args.use_conf
        )
        if args.output_dir and args.save_ckpt:
            utils.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, model_name='latest', model_ema=None,
            )
        if data_loader_val is not None:
            test_stats = validation_one_epoch(data_loader_val, model, device, bf16=args.bf16, accelerator=accelerator)
            ...
            if max_accuracy < test_stats["acc1"]:
                ...
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch=epoch, model_name='best', model_ema=None,
                        # ceph_args=ceph_args,
                    )
            else:
                epochs_no_improve += 1
