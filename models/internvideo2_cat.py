...
try:
    ...
    print("Warning: FlashAttention extensions (FusedMLP, DropoutAddRMSNorm) not found. Using Python fallback.")
    
    class FusedMLP(nn.Module):
        def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., heuristic=1):
            super().__init__()
    ...
    class DropoutAddRMSNorm(nn.Module):
        def __init__(self, hidden_size, p=0.0, eps=1e-5, prenorm=False):
            super().__init__()
            self.p = p
            self.eps = eps
        ...
        def forward(self, x, residual=None):
            ...
            # Apply dropout to x (which is the output of the previous block)
            x = self.dropout(x)
            
            # Add residual
            if self.prenorm:
                residual = residual + x
                x = residual # In prenorm, we normalize the sum
            else:
                x = x + residual
                residual = x # In postnorm, residual is the sum
            ...
            x = self.weight.to(input_dtype) * x.to(input_dtype)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, k=None, v=None):
        ...
        # with open('clip_attn.pkl', 'wb') as f:
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        if self.use_flash_attn:
            ...
            x = self._flash_attn(x)
        else:
            print("Using Naive Attention (High Memory!)")
...
class InternVideo2(nn.Module):
    def __init__(
            ...
            merge_method: str = "cls_avg1_proj",  # proj, cls_avg1, cls_avgN, cls_avg1_proj, cls_avgN_proj
    ...
    def forward(self, x):
        ...
        # Clear attention maps if storing is enabled
        if self.store_attn:
            self.attn_maps = []
        ...
        #     if self.store_attn:
        #         if hasattr(blk.attn, 'attn'):
        #             # Manual collection from the attention block
        #             attn_map = blk.attn.attn
        #             self.attn_maps.append(attn_map.detach().cpu())
        # print(f"Attention maps shape: {len(self.attn_maps)}")

        # import pickle
        # with open('cache.pkl', 'wb') as f:
        #     cache = self.get_attention_maps()
        #     pickle.dump(cache, f)
        ...
        if self.return_features:
            return torch.cat([final, x], dim=1)
    ...
    def get_attention_maps(self, as_numpy=True):
        if not self.store_attn:
            print("Warning: store_attn is set to False. No attention maps are stored.")
            return []
            
        if as_numpy:
            return [attn.numpy() for attn in self.attn_maps]
        return self.attn_maps

@register_model
def internvideo2_cat_small_patch14_224(pretrained=False, store_attn=False, **kwargs):
    model = InternVideo2(
...
@register_model
def internvideo2_cat_base_patch14_224(pretrained=False, store_attn=False, **kwargs):
    model = InternVideo2(
...
@register_model
def internvideo2_cat_large_patch14_224(pretrained=False, store_attn=False, **kwargs):
    model = InternVideo2(
...
@register_model
def internvideo2_cat_1B_patch14_224(pretrained=False, store_attn=False, **kwargs):
    model = InternVideo2(
...
@register_model
def internvideo2_cat_6B_patch14_224(pretrained=False, store_attn=False, **kwargs):
    model = InternVideo2(
