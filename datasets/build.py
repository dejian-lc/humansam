...
from .ssv2 import SSVideoClsDataset, SSRawFrameClsDatasetCombine, SSVideoClsDatasetFromRawFrame, SSVideoClsDatasetCombine
...
def build_dataset(is_train, test_mode, args):
    ...
    if args.data_set in [
        ...
        # Determine suffix based on nb_classes
        suffix = ''
        if args.nb_classes in [2, 4]:
            suffix = f'_{args.nb_classes}'

        if is_train is True:
            mode = 'train'
            anno_path = os.path.join(args.data_path, f'train{suffix}.csv')
        elif test_mode is True:
            mode = 'test'
            anno_path = os.path.join(args.data_path, f'test{suffix}.csv') 
        else:  
            mode = 'validation'
            anno_path = os.path.join(args.data_path, f'val{suffix}.csv') 

        if args.use_decord:
            # func = SSVideoClsDataset
            # func = SSVideoClsDatasetFromRawFrame
            func = SSVideoClsDatasetCombine
        else:
            func = SSRawFrameClsDatasetCombine
