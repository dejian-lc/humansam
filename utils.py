...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, model_name=None, ceph_args={'use_ceph_checkpoint': False}):
    ...
    # Use accelerate's save_state if available (implied by loss_scaler being None or handled externally)
    # But here we are in a mixed context. 
    # If we want to use accelerate's saving mechanism, we should use accelerator.save_state()
    # However, the current function signature doesn't pass 'accelerator'.
    # Let's stick to standard torch.save for now as it's robust for single/multi-gpu (non-FSDP/DeepSpeed).
    
    # Check if the model is a DeepSpeed engine (has save_checkpoint method)
    is_deepspeed = hasattr(model, 'save_checkpoint')
    ...
    if not is_deepspeed:
            ...
            for k, v in model_state_dict.items():
                if v.dtype == torch.float32:
                    if hasattr(args, 'bf16') and args.bf16:
                        model_state_dict[k] = v.to(torch.bfloat16)
                    else:
                        model_state_dict[k] = v.to(torch.float16)
        ...
        # DeepSpeed save_checkpoint handles optimizer state internally
        # We can control it via exclude_frozen_parameters or similar, but usually it saves everything.
        # To strictly follow user request for DeepSpeed, we might need more complex logic, 
        # but for standard PyTorch (loss_scaler is not None), we can easily skip it.
        # For DeepSpeed, it's often better to let it manage its own state for resumption.
        # However, if the user insists on "only weights", DeepSpeed's save_checkpoint might be too heavy.
        # But usually DeepSpeed checkpoints are needed for resumption. 
        # If the user just wants weights for inference, they can extract it later.
        # For now, I will only modify the standard PyTorch saving part as requested.
