...
from engines.engine_for_finetuning_combine import final_test, merge
...
def main(args):
    ...
    if args.finetune:
        if args.return_depth:
             # For HumanSAM, we load the checkpoint directly into the wrapped model
             # assuming the checkpoint contains the full HumanSAM state
             try:
                 checkpoint = torch.load(args.finetune, map_location='cpu', weights_only=False)
             except TypeError:
                 checkpoint = torch.load(args.finetune, map_location='cpu')
             
             print(f"Loading HumanSAM checkpoint from {args.finetune}")
             
             # Handle potential key mismatches if saved with DDP/module prefix
    ...
    if global_rank == 0:
        ...
        final_top1, final_precision, final_recall, final_auc, double_acc, double_auc, num_videos = merge(
            args.output_dir, num_tasks, args.nb_classes)
        print(
            f"Accuracy of the network on the {num_videos} test videos: Top-1: {final_top1:.2f}%, Precision: {final_precision:.2f}%, Recall: {final_recall:.2f}%, AUC: {final_auc:.2f}%")
